...
To install the package, a `.toml` unified Python project settings file is provided. Download, navigate to the folder where the . `.toml` file is located and run `pip install`. Alternatively, the files can be imported as modules locally. 
...
The location of the highlighted pixels will be saved as a .png file and the spectral mask will be saved as a tab-delimited .txt file. An example of this .txt file is provided `example/example_output/example_data_spectra.txt`. In addition, the .txt file saves the **sum** of all spectra. 
